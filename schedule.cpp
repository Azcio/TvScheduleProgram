...
// 获取名字最长度的字符串
int getMaxNameLen(vector<ProgrammeSchedule> &schedules)
{
    size_t maxName = 0;
    for (const auto &program : schedules)
    {
        maxName = max(maxName, program.Name.size());
    }
    return maxName;
}
// 获取名字最长度的字符串
int getMaxChannelLen(vector<ProgrammeSchedule> &schedules)
{
    size_t maxName = 0;
    for (const auto &program : schedules)
    {
        maxName = max(maxName, program.Channel.size());
    }
    return maxName;
}

vector<ProgrammeSchedule> parseData(string EnterFile)
{
    cout << "Parsing data from file: " << EnterFile << endl;
    // using vectors to store the data from cvs file which is in the data structure in structure header
    ...
    {                                                     // while the csvfile is open and readable the programme will read all lines in the file
...
string textCenter(string str, size_t width)
{
...
string textLeft(string str, size_t width)
{
    return str + string(width - str.size(), ' ');
}

string textRight(string str, size_t width)
{
    return str + string(width - str.size(), ' ');
}
...
void DailySchedule(vector<ProgrammeSchedule> schedules)
{
    ...
    string ProgramID = "ProgramID", Channel = "Channel", Name = "Name", StartTime = "Start Time", EndTime = "End Time";
    size_t maxProgramID = ProgramID.size();
    // Get maximum string length for each column
    int programIDLength = maxProgramID,
        channelLength = getMaxChannelLen(schedules),
        nameLength = getMaxNameLen(schedules),
        startTimeLength = StartTime.size(),
        endTimeLength = EndTime.size();
    // display schedule
    cout << textCenter(ProgramID, programIDLength) << " | "
         << textLeft(Channel, channelLength) << " | "
         << textLeft(Name, nameLength) << " | "
         << setw(startTimeLength) << StartTime << " | "
         << setw(endTimeLength) << EndTime << endl;
    ...
    for (const auto &program : schedules)
    {
        ...
        stringstream startTimeStr;
        startTimeStr << setfill('0') << setw(2) << currentTime.tm_hour << ":" << setw(2) << currentTime.tm_min;
        stringstream endTimeStr;
        endTimeStr << setfill('0') << setw(2) << endTime.tm_hour << ":" << setw(2) << endTime.tm_min;
        // Print program details
        int programIDPadding = (programIDLength) / 2;
        int channelPadding = (channelLength) / 2;
        int namePadding = (nameLength) / 2;
        int startTimePadding = (startTimeLength) / 2;
        int endTimePadding = (endTimeLength) / 2;

        stringstream ProgramIDStr;
        ProgramIDStr << setfill('0') << setw(2) << program.ProgramID;
        cout << textCenter(ProgramIDStr.str(), programIDLength) << " | "
             << textLeft(program.Channel, channelLength) << " | "
             << textLeft(program.Name, nameLength) << " | "
             << textCenter(startTimeStr.str(), startTimeLength) << " | "
             << textCenter(endTimeStr.str(), endTimeLength) << endl;
